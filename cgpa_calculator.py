...
sgpas_upto_5 = []
for i in range(5):
    sgpa = st.number_input(f"Semester {i+1} SGPA", min_value=0.0, max_value=10.0, step=0.001, format="%.3f")
...
if all(s > 0 for s in sgpas_upto_5):
    ...
    target_cgpa = st.number_input("🎯 Desired final CGPA after Semester 6", min_value=0.0, max_value=10.0, step=0.001,format="%.3f")
